...
1. **At night only the window changes.** It shows the original night view from before commit 254f596: moon, twinkling stars, city silhouette, blinking lights and slate glazing bars. Nothing else in the room changes. There is no dimming and no light UI for day, and the UI, including the dialogue box, the top bar and the browser chrome colour, is identical in both phases. The Sun-shroom still sleeps by day and wakes at night. Two other versions were built and rejected: a light day UI (`src/day.css`), and a navy dimming layer over the whole room.
