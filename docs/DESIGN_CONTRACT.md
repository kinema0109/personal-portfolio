...
2026-09-16 **prototype, not approved**: the three collectibles are redrawn in code in `src/art/Figures.tsx` instead of being downsampled renders, because the owner found them out of tone with the room. Measured: each sprite carried 252–254 colours against the whole room's 113 and a code-drawn prop's 14. Requantising them to 14 colours (`scripts/flatten-figures.mjs`) changed almost nothing, which showed the problem is spatial rather than palette: a different material on nearly every pixel, where the room is large flat blocks. The Master Ball redraw is good; the Grey Seer and the Alpha Legion are first drafts and are not accepted. Each figure also gained a hover label in its own voice rather than its name. The old sprites and the exporter are untouched, so this is reversible.
