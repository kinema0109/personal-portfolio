...
2026-09-16 owner-approved hover on the sunflower: pointing at the plant outlines the plant, not its click box. The box is 27x61 around a plant that fills about a quarter of it, so the default hotspot rectangle said nothing about what was clickable. Every rectangle of the flower is grown by one unit and painted behind the real one, which leaves a one-pixel rim on the silhouette; the head's rim lives inside the bobbing group so it bobs with the head. The `hotspot-plain` class suppresses the box and is used **only** here — the fan and the suns are box-shaped and their hotspots already hug them, so they keep the default rectangle. Keyboard focus still draws the global outline, which is a keyboard user's only cue.
...
2026-09-16 owner-approved floor line and seated legs: **everything standing in the room lands on y=146** — desk legs, the tower, the paint case, the power strip and now the chair. The chair's castors used to stop at y=138, which made it the one object floating above the floor; its gas column is longer and its base reaches the floor. Thọ's legs were hidden entirely by the chair back, so his trouser cuffs and shoes are drawn either side of the gas column (x 157–167 and 174–184), after the chair so they read in front of it. The chair base, castors and shoes are drawn in mid greys rather than near-black, because the desk casts a shadow across that strip and true black disappears into it. The tower grows from 14x28 to 18x33 at x 206–224, filling the 35 units of clearance under the desk instead of looking undersized beside the drawer.
