...
2026-09-16 owner-approved day and night, and the Sun-shroom: **the room follows the visitor's own day and night, and night changes brightness only, never colour.** Auto reads the browser's time zone and places the visitor at that zone's principal city (`src/daylight/zoneCoords.ts`, generated by `scripts/build-zone-coords.mjs` from tzdb `zone.tab`, `zone1970.tab` and `backward`). It then asks whether the sun is above the horizon there (`src/daylight/solar.ts`). There is no IP lookup and no network call. A ☀/☾ top-bar button swaps the phase; the choice is kept in `tho-vn:phase` only while it differs from the clock.

- **Night** is the daylight room exactly as described above, under a navy dimming layer at 40%. The mask spares only what gives off light, each cut to its own pixels: the laptop screen (but not Thọ's arm and hands in front of it), the tower's lit strips and the lamp's bulb. Rectangular cut-outs were tried first and left bright seams across the Sunflower's head, the arm and the lamp, so do not widen them. The window keeps its daylight view at night. A night view with a moon and stars, and a light UI for day, were both built and rejected.
...
- **Suns** are drawn above the dimming, so they shine at night.
- **Checks:** `scripts/verify-daylight.mjs` and `scripts/verify-sunshroom.mjs`. `verify-hover.mjs` pins day, because the dimming shifts the rim colour it measures.
