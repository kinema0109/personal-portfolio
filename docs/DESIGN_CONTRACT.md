...
2026-09-16 owner-approved wall shelf: the empty wall over the left of the desk now holds the book behind each Limbus Company sinner, twelve sinners plus Dante, drawn by `src/art/BookShelf.tsx` and anchored at `LIMBUS_SHELF` (x=120, floor=46). Spine thickness comes from the length of the work, so the row is uneven the way a real shelf is. Each spine names itself on hover, focus or tap, reusing the Fire Emblem shelf's label mechanism in Scene.tsx. The shelf sits at x>=86 on purpose: the camera crops everything left of that on portrait phones, so the far left wall cannot hold content that must be seen. Binding colours are a choice, not canon; edit the table in BookShelf.tsx to change them.
